jQuery(document).ready(function() {
    ...
    jQuery("#go-up").click(function() {
        jQuery("body,html").animate({scrollTop: 0}, 1e3)
        ...
        jQuery(".bxslider-internal").bxSlider({pagerCustom: "#bx-pager-internal", speed: 1e3, controls: !1, mode: "fade", auto: !0, pause: 5e3})
    }, jQuery.fn.bxSlider && jQuery.internal_custom_bxslider(), jQuery.welcome_custom_bxslider = function() {
        jQuery.fn.bxSlider && jQuery(".bxslider-welcome").bxSlider({speed: 1e3, controls: !0, auto: !0, pause: 5e3, pager: !1})
    }, jQuery.fn.bxSlider && jQuery.welcome_custom_bxslider(), jQuery.fn.owlCarousel && jQuery("#testimonials-slider").owlCarousel({items: 3, itemsTablet: [1024, 1], navigation: !0, pagination: !1}), jQuery.fn.owlCarousel && jQuery("#services-box").owlCarousel({items: 3, itemsTablet: [980, 2], itemsMobile: [480, 1], navigation: !0, pagination: !1}), jQuery.fn.parsley && jQuery("#contact-form").length !== 0 && jQuery("#contact-form").parsley(), jQuery.fn.isMobile && null == isMobile.any() && jQuery("[data-background='parallax']").css("background-size", ""), "rtl" === a ? jQuery(".datepicker-fields").datepicker({format: "yyyy-mm-dd", rtl: !0, language: "ar", }) : jQuery(".datepicker-fields").datepicker({format: "yyyy-mm-dd", }), "rtl" === a ? jQuery("#booking-tab-contents .booking-dates").datepicker({format: "yyyy-mm-dd", rtl: !0, language: "ar", inputs: jQuery('.booking-date-fields-container')}) : jQuery("#booking-tab-contents .booking-dates").datepicker({format: "yyyy-mm-dd", inputs: jQuery('.booking-date-fields-container')}), jQuery("#style-selector-handle").click(function() {
    ...
    jQuery("#style-selector-box [class*=preset_]").click(function() {
        ...
        if ("0" === b) {
            if ("rtl" == a)
                var e = jQuery("#main-style-file").attr("href").replace("styles-rtl_" + d, "styles-rtl");
            else
                var e = jQuery("#main-style-file").attr("href").replace("styles_" + d, "styles");
            jQuery("#main-style-file").attr("href", e)
        } else {
            if ("0" === d)
                if ("rtl" == a)
                    var e = jQuery("#main-style-file").attr("href").replace("styles-rtl", "styles-rtl_" + b);
                else
                    var e = jQuery("#main-style-file").attr("href").replace("styles", "styles_" + b);
            else if ("rtl" == a)
                var e = jQuery("#main-style-file").attr("href").replace("styles-rtl_" + d, "styles-rtl_" + b);
            else
                var e = jQuery("#main-style-file").attr("href").replace("styles_" + d, "styles_" + b);
            jQuery("#main-style-file").attr("href", e)
        }
